...
class TorchWrapperNoDelay(gym.Wrapper):
    ...
    def __init__(self, env):
        ...
        D = np.zeros((self._env.observation_space.sample().shape[0],calibDmCommands.shape[0]))
        print("Doing imat...")
        self._env.step(0*calibDmCommands[0,:],showAtmos = False)
        for i in tqdm.tqdm(range(calibDmCommands.shape[0])):  #tqdm used to create progress bar
            for j in range(2):  #To compensate 1 frame lag of wfs sensor
                slopes_push = self._env.step(calibDmCommands[i,:],showAtmos = False)[0]
            for j in range(2):
                slopes_pull = self._env.step(-1*calibDmCommands[i,:],showAtmos = False)[0]
        ...
        self.action_space = spaces.Box(-5, 5, shape=(self._env.get_total(1), self._env.get_total(1), ), dtype=np.float32)
        self.observation_space = spaces.Box(low=-1, high=1, shape=(1, self._env.get_total(1), self._env.get_total(1)), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self.m1_gain > 0:
            action_vec = np.clip((action_vec * self.normalizer + self.last_action),-0.5,0.5)
        else:
        ...
        action_dm0 = self.last_action_dm0 - self.m1_gain * np.matmul(self.S2V_0, self._env.get_slopes())
    ...
    def process_obs(self,obs,action_dm0):
        obs = self._env.vec_to_img(-1 * np.matmul(self.S2V_1, obs))#-np.matmul(self.V2S_0,action_dm0)))
