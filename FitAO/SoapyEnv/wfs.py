...
class WFS(object):
    ...
    def __init__(self, soapy_config, n_wfs=0, mask=None):
        ...
        self.phase_scale = 1.0 / self.soapy_config.sim.pxlScale
        ...
        # Else we'll just make a circle
            ...
            self.mask = aotools.circle(self.pupil_size / 2.0, self.sim_size)
    ...
    def initLos(self):
        ...
        self.los = lineofsight_legacy.LineOfSight(
            self.config, self.soapy_config, propagationDirection="down"
        )
        ...
        if self.config.lgs:
            ...
            if (
                ...
                self.elongHeights = numpy.linspace(
                    self.config.GSHeight - self.elong / 2.0,
                    self.config.GSHeight + self.elong / 2.0,
                    self.elongLayers,
                )
                ...
                logger.debug("Elong Meta Pupil Pos: {}".format(self.los.metaPupilPos))
            # If GS at infinity cant do elongation
            elif self.config.GSHeight == 0 and self.lgsConfig.elongationDepth != 0:
    ...
    def calcElongPhaseAddition(self, elongLayer):
        ...
        d = numpy.array(self.lgsLaunchPos).astype("float32") * self.los.telDiam / 2.0
        ...
        focalPathDiff = (2 * numpy.pi / self.wfsConfig.wavelength) * (
            (
                ((self.telescope_diameter / 2.0) ** 2 + (h**2)) ** 0.5
                - ((self.telescope_diameter / 2.0) ** 2 + (H) ** 2) ** 0.5
            )
            - dh
        )
        ...
        tiltPathDiff = (2 * numpy.pi / self.wfsConfig.wavelength) * (
            numpy.sqrt((dh + H) ** 2.0 + ((dh + H) * theta - d - D / 2.0) ** 2)
            + numpy.sqrt(H**2 + (D / 2.0 - d + H * theta) ** 2)
            - numpy.sqrt(H**2 + (H * theta - d - D / 2.0) ** 2)
            - numpy.sqrt((dh + H) ** 2 + (D / 2.0 - d + (dh + H) * theta) ** 2)
        )

        phaseAddition = numpy.zeros((self.pupil_size, self.pupil_size))

        phaseAddition += (self.elongZs[2] / self.elongZs[2].max()) * focalPathDiff
        # X,Y tilt
        phaseAddition += (self.elongZs[0] / self.elongZs[0].max()) * tiltPathDiff[0]
    ...
    def calcElongPos(self, elongLayer):
        ...
        xl = numpy.array(self.lgsLaunchPos) * self.telescope_diameter / 2.0
    ...
    def addPhotonNoise(self):
        ...
        """
        self.wfsDetectorPlane = numpy.random.poisson(self.wfsDetectorPlane).astype(DTYPE)
