...
class PyOOMAO(gym.Env):
    ...
    def __init__(self):
        ...
        if not os.path.isfile(os.getcwd()+"/env.m"):
            try:
                self.eng.cd(os.getcwd()+"/OOMAOEnv")    #Change matlab folder to OOMAO env folder
            except:
    ...
    def step(self, action, showAtmos = True):
        ...
        slopes, done, info = self.eng.step(self.env, showAtmos, nargout = 3,stdout=io.StringIO())
    ...
    def render(self, mode='rgb_array'):
        dm, ngs, atmos, img, wfs_img = self.eng.render(self.env,nargout=5,stdout=io.StringIO())
        ...
        try:
            ...
            plt.setp(plt.gcf().get_axes(), xticks=[], yticks=[])  #Remove axis ticks for readability
    ...
    def reset(self, seed = -1): #-1 random seed (default), > 0 set seed
        ...
        self.action_buffer = []
        slopes = self.list2python(self.eng.rst(self.env, nargout = 1,stdout=io.StringIO()))/(self.get_wavelen()/1000000)
    ...
    def set_params(self,seed = -1): #-1 random seed (default), > 0 set seeds
        ...
        self.eng.set_tel_params(self.env,self.tel_params["diam"],self.tel_params["obstructionRatio"],self.tel_params["samplingTime"], self.tel_params["pupdiam"], nargout = 0,stdout=io.StringIO())
        if ("alt" in self.gs_params) and (self.gs_params["alt"] > 0):   #LGS
            self.eng.set_lgs_params(self.env,self.gs_params["magnitude"],self.gs_params["wavelength"],self.gs_params["alt"],self.gs_params["lltx"],self.gs_params["llty"], self.gs_params["xpos"], self.gs_params["ypos"], nargout = 0,stdout=io.StringIO())
            print("LGS")
        else:   #NGS
            self.eng.set_ngs_params(self.env,self.gs_params["magnitude"],self.gs_params["wavelength"], nargout = 0,stdout=io.StringIO())
            print("NGS")
        self.eng.set_science_params(self.env,self.science_params["magnitude"],self.science_params["wavelength"], nargout = 0,stdout=io.StringIO())
        if self.wfs_params["type"] == "sh":
            self.eng.set_sh_params(self.env,self.wfs_params["nxsub"],self.wfs_params["npix"],self.wfs_params["fracsub"],self.wfs_params["noise"], nargout = 0,stdout=io.StringIO())
        else:
            self.eng.set_pyr_params(self.env,self.wfs_params["nxsub"],self.wfs_params["fssize"],self.wfs_params["fracsub"],self.wfs_params["noise"],self.wfs_params["pyr_amp"],self.wfs_params["pyr_pup_sep"],self.wfs_params["pyr_npts"],self.tel_params["pupdiam"], nargout = 0,stdout=io.StringIO())
        self.eng.set_dm_params(self.env, self.wfs_params["nxsub"], self.tel_params["pupdiam"], self.dm_params["coupling"], nargout=0,stdout=io.StringIO())
        self.eng.set_atmos_params(self.env, self.atmos_params["windSpeed"], self.atmos_params["windDirection"], self.atmos_params["r0"], self.atmos_params["layeredL0"], self.atmos_params["fractionalR0"], self.atmos_params["altitude"], seed, nargout = 0,stdout=io.StringIO())
    ...
    def _param_file_parser(self):
        ...
        self.tel_params,self.atmos_params,self.gs_params,\
        self.science_params,self.wfs_params,self.dm_params,self.delay\
        ...
        self.atmos_params["r0"] = (self.atmos_params["r0"]/(500**(6/5)))*(550**(6/5))    #r0 from 500nm -> 550nm used by OOMAO
    ...
    def get_dm_shape(self,dm_idx=0):
        return np.asarray(self.eng.get_dm_shape(self.env,nargout=1,stdout=io.StringIO()))

    def get_phase_screen(self,target=0):
        return np.asarray(self.eng.get_phase_screen(self.env,nargout=1,stdout=io.StringIO()))

    def get_pupil(self):
        return np.asarray(self.eng.get_pupil(self.env,nargout=1,stdout=io.StringIO()))

    def get_KL_modes2volt(self,n_modes=None):
        mod2volt = np.asarray(self.eng.get_KL_modes2volt(self.env,nargout=1,stdout=io.StringIO()))
    ...
    def _get_photonpsubap(self):
        total_photons = self.eng.get_photonpsubap(self.env,nargout=1,stdout=io.StringIO());
        print(total_photons)
    ...
    def wavelength_warning(self,txt,wavelength):
        ...
        # OOMAO.
        supported_wavelength = [0.36, 0.44, 0.55, 0.64, 0.79, 1.22, 1.65, 2.18, 3.55, 4.77]
        ...
        if closest == wavelength:
            ...
            print("\033[93m"+txt+" set wavelength of "+str(wavelength)+" micrometers is not supported by OOMAO!")
            print("Using the closest supperted wavelength of "+str(closest)+" for OOMAO. \n (Consider changing the simulation parameter file to match)"+'\x1b[0m')
            time.sleep(5)
