...
class CompassEnv(gym.Env):
    ...
    def render(self, mode='rgb_array'):
        ...
        plt.imshow(self.supervisor.target.get_tar_image(0,expo_type="se"))  #expo_type = "le" for long exposure
        plt.xlim(465,565)
        plt.ylim(465,565)
        plt.title('Image')
    ...
    def _init_supervisor(self):
        ...
        try:
            ...
            self.xvalid = ((xvalid - xvalid.min()) / self.supervisor.config.p_dms[1]._pitch).astype(np.int)
            self.yvalid = ((yvalid - yvalid.min()) / self.supervisor.config.p_dms[1]._pitch).astype(np.int)
            #
            ...
            # + 2 for tip/tilt DM
            n_actions = self.get_valid(1)#valid_actus[self.xvalid, self.yvalid].shape[1]# + 2
    ...
    def get_KL_modes2volt(self,n_modes=None):
        mb = ModalBasis(self.supervisor.config,self.supervisor.dms,self.supervisor.target)
    ...
    def vec_to_img(self, action_vec):
        valid_actus = np.zeros((self.supervisor.config.p_dms[1].nact , self.supervisor.config.p_dms[1].nact))
