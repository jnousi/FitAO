...
class control():
    ...
    def closed_loop(self,obs, gain=0.6):
        ...
        if not self.skip_WFS:
            ...
            if len(self.actions) == 0 and self.pmat is not None:
                self.actions.append(0 * np.matmul(self.pmat,self.env.get_phase_screen(0).flatten()))
            ...
            self.actions.append(self.actions[-1] - gain * np.matmul(self.pmat,self.env.get_phase_screen(0).flatten()))
    ...
    # Gain can be used to limit overshoots by errors in estimates.
        if not self.skip_WFS:
            ...
            if len(self.actions) == 0 and self.pmat is not None and self.infmat is not None:
                self.actions.append(0 * np.matmul(self.pmat,self.env.get_phase_screen(0).flatten()))
            elif (self.pmat is None or self.infmat is None):
                print("Influence/projection matrix might be missing!")
                time.sleep(5)

            openloop_obs = self.env.get_phase_screen(0).flatten() - np.matmul(self.infmat,self.actions[-1])
